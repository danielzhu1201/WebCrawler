...
/**
 * A very ugly and basic implementation of an HTTP web server which provides a
 * search bar and responds to simple search queries.
 *
...
public class WebServer {
	...
	/**
	 * main method to start a server. Loads a WebIndex from the default save
	 * location.
	 */
	 ...
	 */
	public void serve() throws IOException, ParseException {
		int port = 1989;
	 ...
	 * @throws ParseException
	 */
	private void renderSearchResults(BufferedWriter out, HashMap<String, String> params)
			throws MalformedURLException, ParseException {
		String content = "<TITLE>Results</TITLE>" + "<body> <div style='width:800px; margin:0 auto;'>" + "<a href='/'>"
				+ layoutLogo() + "</a>";

		// Probably add a disclaimer about file links
		Collection<Page> results = new ArrayList<>();
		try {
			results = engine.query(params.get("query"));

		} catch (Exception e) {
			e.printStackTrace();
			System.err.println("Incorrect Query Input in browser");
		}
		...
		if (results.size() > 0) {
			content += "<p>Here are the results of your query.<br> " + "Some browsers don't follow file:// links "
		...
		content += "<ul>\n";
		for (Page p : results) {
			content += "<li>" + layoutPageContent(p) + "\n<li>" + p.getPreview() + "     [Page Score:" + p.getScore()
					+ "]" + "</li>" + "</li>\n";
		}
		content += "</ul>\n" + "</div> </body>";
	...
	private void renderHomepage(BufferedWriter out) {
		String content = "<TITLE>Home</TITLE>" + "<body> <div style='width:800px; margin:0 auto;'>" + layoutLogo()
	...
	/**
	 * This method is going to try and create the logo on the page by loading an
	 * image file. It tries to open the image on each request instead of holding
	 * onto it, which isn't ideal.
	 */
	private String layoutLogo() {
		try {
			...
			return "<img src='data:image/png;base64," + base64String + "' style='display: block; margin:0 auto;'>";
		} catch (IOException e) {
	...
	/**
	 * We'll use this to get the HTML that represents our Page object. It might be
	 * better to Encapsulate this within the Page class itself.
	 */
	...
	/**
	 * This method pretends to be a real webserver by sending valid HTTP headers in
	 * front of the content that we're responding with.
	 */
	...
	private class HttpRequest {
		...
		HttpRequest(String u, String m, HashMap<String, String> h, HashMap<String, String> p) {
			url = u;
			method = m;
			headers = h;
			params = p;
	...
	/**
	 * Returns a HttpRequest object containing the information from the next section
	 * of the input stream, or null if we can't understand it.
	 */
